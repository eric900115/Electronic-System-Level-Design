...
### Problem Formulation
- Implement a Gaussian Filter using a systemC module.
- Connect the testbench and Guassian Filter module using FIFO channels.

### Block Diagram
![image](https://hackmd.io/_uploads/rJutAJKy0.png)
...
The code section provided below demonstrates `do_filter` process within GuassianFilter module. This process reads a 5x5 pixel block from the image, received from the testbench through FIFO channels(r, g, b). The process then performs convolution operation with a 5x5 Gaussian Filter. After calculating the convolution operation, it outputs the result back to testbench through the result FIFO channel.
...
```
#### Testbench
The provided code section below demonstrates the `do_gaussian` process within the Testbench module. This process send 5 x 5 pixel data blocks to the GaussianFilter module through FIFO channels for each iteration to calculate a single output pixel. After the computation, it reads the calculated result from the GuassianFilter module through the result FIFO channel.
...
### Problem Formulation
- Implement a Gaussian Filter using a SystemC module, incorporating an additional input buffer. 
...
### Block Diagram
![image](https://hackmd.io/_uploads/B1wMKbFyC.png)
...
Here, I outline compare the difference in the methods to prepare 5x5 pixel data block between Part2 and Part1.

In part1, for each output pixel that needs processing, the GuassianFilter module reads a full 5x5 pixel block directly from the image. The 5x5 pixel block is received from the testbench through FIFO channels(r, g, b). 

However, in part2 additional buffer is introduced to reduce data transfer. If the current processing element is the first element of a new row, the testbench sends a complete 5x5 pixel block, which is similar to part1. For subsequent pixels in the same row, the testbench sends only 5x1 pixel data block. This efficiency is possible because the remaining 5x4 pixels of the data block can be reused from buffer, which stores the data from previous cycle.

#### Testbench
The provided code section below illustrates the `do_gaussian` process within the Testbench module. This process send 5 x 5 pixel data blocks to the GaussianFilter module through FIFO channels for each iteration to calculate a single output pixel, if it is first element of new row. For the remaining elements in the row, the testbench send only 5 x 1 pixels. After the computation, it reads the calculated result from the GuassianFilter module through the result FIFO channel.
```C
...
- Part1 (without buffer) : To calculate each output pixel, it requires 5x5 + 1 data transfer. Considering there are 256 x 256 pixels in the case, it requires 256 x 256 x (5 x 5 + 1) data tansfers.
- Part2 (with buffer) : To calculatethe first pixel of a row, it requires 5x5 + 1 data transfer. The remaining pixels require only 5 + 1 data transfer. Given there are 256 rows and 256 columns, it requires 256 x (5x5 + 1) + 256 x 255 x (5 + 1) data tensfers.
- The amountof data transfer for Part1 is (256 x 256 x (5 x 5 + 1))/(256 x (5x5 + 1) + 256 x 255 x (5 + 1)) = 4.28 times greater than Part2.
