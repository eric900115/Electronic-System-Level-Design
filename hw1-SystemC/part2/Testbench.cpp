...
int Testbench::read_bmp(string infile_name) {
  ...
  target_bitmap =
      (unsigned char *)malloc((size_t)width * height * 1);
...
int Testbench::write_bmp(string outfile_name) {
  ...
  // file size
  file_size = width * height * 1 + 54 + 1024; //FIX
  ...
  for (int i = 0; i < 256; i++) {
    // For grayscale, R=G=B=i, A=0 (not used in BMP palette)
    unsigned char color[4] = {(unsigned char)i, (unsigned char)i, (unsigned char)i, (unsigned char)0};
    fwrite(color, sizeof(color), 1, fp_t);
  }

  // write image
  fwrite(target_bitmap, sizeof(unsigned char),
         (size_t)(long)width * height * 1, fp_t); //FIX
...
void Testbench::do_gaussian() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      *(target_bitmap + 1 * (width * y + x)) = total; //FIX
      //*(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = total; //FIX
      //*(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = total; //FIX
      //cout << total << " ";
    }
   // cout << "\n";
  }
